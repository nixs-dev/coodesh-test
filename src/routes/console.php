        ...
        try {
            ...
            while (!gzeof($stream) && $imports_count < $max_imports) {
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    $this->error('Erro ao decodificar a linha JSON: ' . json_last_error_msg());
                }
            }

            gzclose($stream);
            
            $this->info("Foram importados $imports_count produtos do arquivo $file");
        } catch (Exception $e) {
            $this->error($e->getMessage());
        }
    }
})->describe('Importa produtos diariamente.');

Schedule::command('products:import')->dailyAt('00:00');